...
```xml
<ItemGroup>
    <ProjectReference Include="..\Launcher-main\CassieLauncher.csproj">
...
**not** end up with a copy of `CassieLauncher.dll` (or the Avalonia
...
`CassieLauncher.dll`, that copy gets loaded into your plugin's
...
output folder's DLLs (minus `CassieLauncher.dll` itself) into
